...
class Kiosk
{
    protected List<MenuItem> menuItems = new ArrayList<>(); // Lv4 요구사항에서 삭제
    ...
    void start()
    {
        ...
        {
            this.input = scanner.next(); // Lv_1_input_01_N: 서브루틴 제어 문자열로 받아들이기

            if(checkNumberException())
            {
                continue;
            }
            ...
            switch(ctrlInt) // Lv_1_input_01_F: 입력받은 값에 따라 해당하는 서브루틴 실행
            {
                case 1: // Lv_1_output_01_F: 햄버거 메뉴 전체 출력
                    ...
                    {
                        // 이중참조 pointer -> menuitems -> menuItem.itemName...
                        pointer = iterator.next(); // interator -> menuitems
                        System.out.println( (this.menuItems.indexOf(pointer) + 1) + ". " + pointer.itemName + "    | "
    ...
    private boolean checkNumberException()
    {
        try
        {
